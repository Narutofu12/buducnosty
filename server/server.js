...
wss.on("connection", ws => {
    ...
    ws.on("message", raw => {
        ...
        if (type === "friendAccept") {
            ...
            // pošalji svima update
            const targetWs = sockets.get(to.uuid);
            if (targetWs && targetWs.readyState === WebSocket.OPEN) {
                targetWs.send(JSON.stringify({ type: "friendAccept", fromProfile: from }));
            }
            const fromWs = sockets.get(from.uuid);
            if (fromWs && fromWs.readyState === WebSocket.OPEN) {
                fromWs.send(JSON.stringify({ type: "friendAccept", fromProfile: to }));
            }
